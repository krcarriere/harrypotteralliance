...
hpa_rating_histo <- ggplot2::ggplot(data=subset(data, !is.na(hpa_rating_1)), aes(x=hpa_rating_1)) + 
  geom_bar(col="black", stat="count", fill=extended_ravecol(4)) + 
  xlab("") + ylab("Frequency") + labs(title="Rate HPA on Lightning Bolt Scars") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
aggTots <- ddply(data, c("consider_wactivist", "female"), summarise,
                 ...
                 N_hparating   = sum(!is.na(hpa_rating_1)),
                 mean_hparating = mean(hpa_rating_1, na.rm=TRUE),
                 sd_hparating   = sd(hpa_rating_1, na.rm=TRUE),
                 se_hparating   = sd_hparating / sqrt(N_hparating),
...
hpa_rating <- ggplot(aggTots, aes(x=consider_wactivist, y=mean_hparating, group=female))+
  geom_bar(stat="identity", position="dodge", aes(fill=female))+
  labs(y="I have protested something I wouldn't have\n because of The HPA!", x="Do you see yourself as a Wizard Activist?", fill="")+
  theme(plot.title = element_text(hjust = 0.5)) + 
  scale_fill_manual(values=c("#d2ad33", "#a1a1a1"))+
  geom_errorbar(aes(ymin=mean_hparating-se_hparating, ymax=mean_hparating+se_hparating),
                width=.3,                    # Width of the error bars
                position=position_dodge(.9))
